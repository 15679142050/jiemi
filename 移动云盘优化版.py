...
def send_notification_message_collection(message):
    """发送通知函数"""
    fn_print(f"通知消息: {message}")
    # 这里可以添加实际的通知发送逻辑，比如企业微信、钉钉等
    pass
...
async def main():
    if not ydyp_ck:
        fn_print("❌ 请先设置环境变量 ydyp_ck")
        return
    
    tasks = []
    for ck in ydyp_ck:
        if not ck or len(ck.split("#")) < 3:
            fn_print(f"❌ Cookie格式错误，跳过: {ck}")
            continue
